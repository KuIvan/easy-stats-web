...
export default function GameActionCreator({ teams, gameId, handleElement }: Props) {
  ...
  useEffect(() => {
    const isAction = ['red_card', 'yellow_card', 'foul', 'steal', 'interception', 'dribbling'].includes(selectedAction)
    if (isAction) {
      setSecondTeam(teams[1])
    } else {
      setSecondTeam(teams[0])
    }
    const isActionSolo = ['goal', 'losing_the_ball', 'position_error'].includes(selectedAction)
    if (isActionSolo) {
      setSecondTeam(null)
    }
    ...
    <Grid container justifyContent='center' spacing={5}>
      ...
      <Grid item xs={12}>
        <Grid container justifyContent='center'>
          <Button variant='outlined' onClick={() => {onCreateNewAction()}}>Add Record</Button>
        </Grid>
      </Grid>
    </Grid>
  )
