...
export default function AddStatisticPage({ gameId }: AddStatisticPageProps) {
  ...
  const [playerOne, setPlayerOne] = useState<number>(null);
  const [playerSecond, setPlayerSecond] = useState<number>(null);
  ...
  function newSelectInput() {
    // axiosClient.post('/#', { playerOne, playerSecond, action })
      // .then(
    const newArray = [
    ...rows,
      {id: rows.size, initiator: playerOne, addressable: playerSecond, action: action }
    ]
        setAction('')
        setPlayerOne(0)
        setPlayerSecond(0)
              ...
              <Box sx={{ minWidth: 120 }} key={form.id}>
                <FormControl fullWidth>
                  ...
                  <Select
                    value={form.value}
                    label={form.label}
                    onChange={(event) => handleChange(event, form.setValue)}
                  >
